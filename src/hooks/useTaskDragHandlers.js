...
export default function useTaskDragHandlers(
  tasks,
  setTasks,
  setActiveTask,
  setDraggingTaskId,
  ...
  const handleDragStart = ({ active }) => {
    ...
    if (status) {
      ...
      const originalIndex = tasks[status].findIndex(
  ...
  const handleDragOver = useCallback(
    debounce(({ active, over }) => {
      ...
      if (activeStatus !== overStatus) {
        const movedTask = tasks[activeStatus].find(
          (task) => task.id.toString() === activeId
        );

        setTasks((prev) => {
          const newActive = prev[activeStatus].filter(
            (task) => task.id.toString() !== activeId
          );
          const newOver = [...prev[overStatus]];
          const overIndex = newOver.findIndex(
  ...
  const handleDragEnd = ({ active, over }) => {
    ...
    if (
      ...
      tasks[activeStatus].findIndex((t) => t.id.toString() === activeId) ===
